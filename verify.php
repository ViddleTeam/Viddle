...
if ($result = @$connect->query(sprintf("SELECT * FROM viddle_ver WHERE vid='%s'", mysqli_real_escape_string($connect,$id)))) {
    $d2 = $result->num_rows;
    if($d2 == '0') {
      $exist = '0';
    }
				...
				<div class="card-body">
                    ...
                    <?php if($exist == '1') { ?>
          			...
          			<?php } else { ?>
						...
						<p>Link jest nieprawidłowy, lub został już wykorzystany do weryfikacji innego konta.</p><br>
					<?php } ?>
        		</div>
				</div>
				</center>
