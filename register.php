...
if (isset($login)){
    ...
    $sk = "6LfI8fwZAAAAAIbM-pHAFeKlHPBt-sMxhypcEycd";
        ...
        require("danesql.php");
            ...
            if ($ok == true) {
                $haslo_h = password_hash($connect->htmlentities(real_escape_string($password), PASSWORD_DEFAULT));
                $login_e = $connect->htmlentities(real_escape_string($login));
                $email_e = $connect->htmlentities(real_escape_string($email));
                $uid = uniqid(uniqid());
